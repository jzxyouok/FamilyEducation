...
<body style="overflow: scroll; margin: 0px auto;">
				...
				<tr>
					...
					<th style="width: 208px; height: 208px;"><a href="#"
						rel="${waitingValidateTeacher.idImage
							...
							style="width: 188px; height: 188px;"></a></th>
					<th style="width: 208px; height: 208px;"><a href="#"
						rel="${waitingValidateTeacher.teachImage
						}" class="preview"><img
							src=${waitingValidateTeacher.teachImage
						}
							style="width: 188px; height: 188px;"></a></th>
					<th style="text-align: center; vertical-align: middle;">
						<a style="margin-right: 20px;"
						class="btn btn-info" onClick="return confirm('点击确认通过审核？')"
						href="/FamilyEdu/admin/checkedSuccess/${waitingValidateTeacher.id}/教师资格">审核通过</a>
						<a
						class="btn btn-info failBtn" data-toggle="modal"
						data-target="#myModal"
						name="/FamilyEdu/admin/checkedFail/${waitingValidateTeacher.id}">审核不通过</a></th>
				</tr>
			</c:forEach>
		</tbody>
	</table>
	...
	<ul class="pagination">
		<c:if test="${sessionScope.teachersCurrentPage!=1}">
			<li><a
				href="/FamilyEdu/admin/findWaitingValidateTeachers?page=${sessionScope.teachersCurrentPage}&pageAction=pre"
		...
		<c:forEach begin="${sessionScope.teachersBeginPage}"
			...
			<c:if test="${ v==teachersCurrentPage}">
				<li class="active"><a
					href="/FamilyEdu/admin/findWaitingValidateTeachers?page=${v }">${v}</a></li>
			</c:if>
			<c:if test="${ v!=teachersCurrentPage}">
				<li><a
					href="/FamilyEdu/admin/findWaitingValidateTeachers?page=${v }">${v}</a></li>
		...
		<c:if
			...
			<li><a
				href="/FamilyEdu/admin/findWaitingValidateTeachers?page=${sessionScope.teachersCurrentPage}&pageAction=next"
